...
| Autonomous Evolution Nursery Runner | `backend.services.autonomous_evolution_runner` + `/api/ops/autonomy/evolution-cycle/run` + `scripts/learning_worker.py` job `autonomous_evolution_nursery` | 稳定 demo nursery 自进化协调器；按周期状态复用 replay harness、release control、candidate review、effect reconcile、Proposal Registry 和既有 demo apply 链；默认只修复 freshness/ready 状态并暴露 `guarded_demo_apply_window`，不自动跑 replay 或 blocking apply；learning worker 可通过 `consume_recommended_step` 每轮最多消费 1 个 stepper 推荐小步，默认允许 bridge/review/conflict/rollback 这类治理推进，执行优先级是 review/conflict/rollback 先于 bridge，避免只桥接不审查，不自动 sync factor weights；`apply_demo_autonomy` 必须显式 `apply_when_ready=true` + `confirm_blocking_apply=true`，完整 `run_autonomous_learning_cycle` 还必须 `full_learning_cycle=true`；手动/API replay freshness 使用轻量 `bar_replay_freshness`，bar 读取遇 DuckDB 写锁时走只读 snapshot fallback；不下单、不绕过 `RiskPolicyService` / `DecisionPolicy` / runtime overlay/snapshot，不新建第二套执行链 |
