...
- live session 风控统计以 PostgreSQL `ctrader_deals` 按最终平仓日、position 聚合重建；`runtime_kv[live.session_state.*]` 只作为进程恢复缓存，不能覆盖 broker close facts。
...
| 模型权限 | `model_permission_audit` / `backend.services.model_permissions` | shadow/advisory guardrails；模型本身不拥有执行权，demo nursery 的模型建议只能通过既有治理服务进入受控 `policy_suggestion` |
...
- V16 Phase 4 medium-impact governance 只 materialize `brain_governance_candidate` 候选和 `brain_medium_impact_governance` 审计账本。它不能直接调用 `_update_weights()`、不能激活模板、不能推广模型到 live、不能写 runtime overlay/snapshot，也不能由候选来源直接写 `policy_suggestion`；权重类候选必须带 `DecisionPolicy` preview。候选进入旧自治建议队列时，`demo_nursery` 由 `AutonomousEvolutionNurseryRunner` 自动调用既有 bridge service；非 demo 仍保留显式 bridge 入口。两种路径都必须满足 `RuleEvolutionGovernor` 可理解 evidence、`RiskPolicyService`、`DecisionPolicy`、release evidence 和 rollback JSON。
...
- V16 governance candidate review 由系统在 `demo_nursery` 自动执行，review service 本身仍只审查、不提交候选。`BrainGovernanceCandidateReviewService` 复用现有 `GovernanceConflictResolver` 的 control surface 口径、复用 candidate bridge preview，并可选调用现有 `LLMAdvisoryService` 写 advisory audit；LLM 不能改变 review status、不能授权 bridge 或执行。demo runner 只会把 `bridge_ready=true` 的候选交给 `BrainGovernanceCandidateService.submit_candidate_to_policy_suggestion()`，并以 system bridge actor 记录；非 demo 的 Ops submit 仍要求已有单候选 review。提交到 `policy_suggestion` 的 evidence 必须携带 review 引用和 `candidate_review_required_before_submit=true`，否则 readiness 视为新合同缺审查。
...
- `FactorGovernanceLightGBMService.materialize_demo_governance_advisories` 在 `demo_nursery` 只为当前 `used_in_score=true` 且仍启用、可进入 live 的 alpha 因子筛选至少 2 条、平均 weakness 不低于 0.85 的模型证据，生成带 `downweight` 的 `policy_suggestion`；模型仍标记 `advisory_only`，实际批准、权重变更、配置快照和效果回滚必须分别经过 `RuleEvolutionGovernor`、`DecisionPolicy`、`RiskPolicyService`、`FactorWeightChangeService` 和 `learning_application_effect`。若因子在建议生成后进入 quarantine/disabled，旧模型建议自动标记 `superseded`，不得把失效目标计为已采用。
