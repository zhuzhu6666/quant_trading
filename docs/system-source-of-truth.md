...
| 静态发布开关 | `backend.core.static_feature_flags.StaticFeatureFlags` + `settings.yaml[features]` / 显式环境变量 | 进程启动时只解析一次，不进入 RuntimeConfig overlay，不允许自治治理修改。当前默认固定为 `live_safety_plane_v2_mode=off`、`live_generation_controller_v2_enabled=false`、`ctrader_execution_outcome_v2_enabled=false`、`governance_mutation_coordinator_v2_mode=off`、`pg_job_queue_v2_enabled=false`；任何切换都必须跟随发布配置与受控重启，不得根据单测结果自动切 enforce |
...
| 本地执行安全闸 | `backend.services.live_safety_state` + `data/safety/no_new_risk_latch.jsonl` + `safety_outbox.jsonl` | emergency 先 fsync append-only `emergency_resume` cause，再与 open admission lock 线性化；incident、broker unknown、safety freshness、emergency-resume 与 V2 forced-shadow 分别锁存。释放必须指定 cause；`broker_execution_unknown` 还必须由 fresh recovery/reconcile 得出 confirmed/rejected 并追加 resolution event，incident thaw 不能代替。PG/治理审计不可用时 close/reduce/tighten 不被阻断，失败证据进入本地 safety outbox。该账本不替代 fresh broker reconcile 的仓位事实或 PostgreSQL committed 治理事实 |
