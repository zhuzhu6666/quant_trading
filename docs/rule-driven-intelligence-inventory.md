...
| 3 | live 信号决策流水线 | `backend/services/live_service.py`、`backend/services/live_decision_pipeline.py`、`alpha/execution_gate.py` | `LiveDecisionFrame`、pass/block、reason、cooldown、`decision_bar_freshness.v1` | factor values、normalized signals、CompositeSignal、context policy effect、threshold、score、direction、cooldown、latest/expected closed bar、repair status；live 下 NFP/GVZ 只形成 `event_filter` 风控输入 | 只产出交易信号候选和 gate verdict；只使用最新已闭合 bar；不读账户、不做 sizing、不调用 `RiskPolicyService`、不下单；事件风险和 stale decision bar 最终由 `RiskPolicyService` 裁决 |
