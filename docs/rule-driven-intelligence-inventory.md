...
v16 medium-impact governance
  -> brain_medium_impact_governance + brain_governance_candidate
  -> RiskPolicy verdict + DecisionPolicy preview + rollback/release requirements + manual policy_suggestion bridge
...
| 3 | live 信号决策流水线 | `backend/services/live_service.py`、`backend/services/live_decision_pipeline.py`、`alpha/execution_gate.py` | `LiveDecisionFrame`、pass/block、reason、cooldown、`decision_bar_freshness.v1`、`last_processed_decision_bar_ts` | factor values、normalized signals、CompositeSignal、context policy effect、threshold、score、direction、cooldown、latest/expected closed bar、repair status；live 下 NFP/GVZ 只形成 `event_filter` 风控输入 | 只产出交易信号候选和 gate verdict；只使用最新已闭合 bar，且同一根已闭合 bar 只推进一次 signal/open；不读账户、不做 sizing、不调用 `RiskPolicyService`、不下单；事件风险和 stale decision bar 最终由 `RiskPolicyService` 裁决 |
...
| 5 | Kelly 动态仓位 | `backend/services/live_risk_sizing.py` | base API volume、sizing trace | `position_sizing_trace.v1`、equity、Kelly fraction、SL distance、risk budget、broker min/step/max | `kelly_risk_per_trade_pct` 作为有效 Kelly 风险分数 cap，不再和 Kelly 分数二次相乘；raw volume 由当前 equity 风险预算推导，demo 硬顶默认 1000 API；非 `demo_nursery` 下非正 Kelly、缺 equity 或低于最小量输出 0 volume/blocked trace；`demo_nursery` 下非正 Kelly 用 broker 默认最小单探索并记录 trace |
| 6 | open-trade candidate sizing | `backend/services/live_service.py::_prepare_open_trade_candidate` | final requested API volume | base volume、event adjusted volume、context adjusted volume、blocked reason、candidate snapshot | 低于最小量时不静默抬回，由后续 risk policy 决定；context 缩仓高于最小量时按最近 broker step 映射，避免过度降档；candidate 只承载执行输入，不授权下单 |
| 7 | 动作级风险策略 | `risk/policy_service.py` | `RiskVerdict` for open/close/reduce/template/factor/model actions | action、context、allowed、reason、severity、audit payload、`risk_limit_snapshot.v1`、`runtime_health_snapshot.v1`、`decision_freshness` | 唯一动作裁决入口；新增自治动作必须在这里注册；事件风险、stale decision bar 和模型权限都在这里统一裁决 |
| 8 | 账户/系统硬风控 | `risk/governor.py` + `risk/runtime_policy.py` | allow/block | drawdown、daily loss、trade limit、demo learning trade limit、data lag、disk、bridge、circuit breaker、RiskLimitSnapshot | fail closed；阈值从 `RiskLimitSnapshot` 输入；`demo_autonomous` 与 `demo_nursery` 都使用显式 demo 日交易上限，不允许以 0 形成无限采样，`force_dry_run`、断连、严重数据延迟优先阻断 |
...
| 50 | V16 medium-impact governance | `backend/services/brain_medium_impact_governance.py`、`backend/services/brain_governance_candidates.py` | medium-impact isolated governance candidates、RiskPolicy verdict、DecisionPolicy preview、rollback/release requirement、governed bridge | `brain_medium_impact_governance`、`brain_governance_candidate`、`brain_action_plan_eval`、`RiskPolicyService` verdict、`DecisionPolicy` preview、readiness `v16.medium_impact_governance` / `v16.governance_candidates`、`/api/ops/brain/medium-impact-governance/materialize`、`/api/ops/brain/governance-candidates/{candidate_id}/submit` | Phase 4 中等影响治理候选；只写隔离候选和审计，不直接写 `policy_suggestion`，不应用权重/模板/模型 promotion，不写 runtime overlay/snapshot、订单或学习样本；demo nursery 由系统自动 bridge，非 demo 走显式 governed bridge，future apply 仍走受控治理写入口 |
...
| 41 | factor governance LightGBM | `research/factor_governance_lightgbm.py` | factor weakness shadow audit / advisory；demo nursery 受控 `downweight` 建议 | factor、weakness_score、bucket、features、audit row、bridge evidence | 模型不单独写权重；demo 仅对仍启用且可进入 live 的活跃 alpha、至少 2 条弱样本且平均 weakness >= 0.85 的证据生成建议；目标随后失活则自动 supersede，最终仍由 Governor/DecisionPolicy/RiskPolicy/FactorWeightChangeService 执行 |
...
| 交易复盘 | `trade_outcome_review`、`factor_contribution_review` | realized pnl、真实 fill-based duration、MFE/MAE、factor attribution、entry timing、decision freshness、system contamination | 成熟后才能给 open outcome / contribution label；系统污染样本只审计/低权重，不作为高置信因子治理训练 |
