...
| 8 | 账户/系统硬风控 | `risk/governor.py` + `risk/runtime_policy.py` | allow/block | drawdown、daily loss、trade limit、demo learning trade limit、data lag、disk、L2、bridge、circuit breaker、RiskLimitSnapshot | fail closed；阈值从 `RiskLimitSnapshot` 输入；`demo_autonomous` 与 `demo_nursery` 都使用显式 demo 日交易上限，不允许以 0 形成无限采样，`force_dry_run`、断连、严重数据延迟优先阻断 |
