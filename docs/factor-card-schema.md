...
> 2026-08-10 的因子链路/发现/治理引擎专项核对（原 7-9 约 490 行，含 state_v1 21因子/12voter 快照、vol_ma_ratio 角色、模型 AUC 0.524 等 08-10 运行态）已归档 Git 历史（commit 5a4e4db 前）。
> 当前因子运行事实以 runtime_kv runtime_factor_selection.v1 + factor_health + factor_lifecycle_state 为准；执行历史以 canonical_v2 为准。
> 需要追溯时用 git show 查看历史版本，不恢复为活动文档。
