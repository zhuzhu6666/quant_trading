...
- **运行态与事实域**：运行态统一 PostgreSQL `runtime`，不可变事实与学习样本统一 `canonical_v2`；migration ledger 当前 v37（`0037_recovery_attribution_integrity`）。旧 `state_v1` / `public` / `legacy_mapping` 和 SQLite `data/state.db` 运行路径已退役，磁盘上的 0 字节残留不是可用入口。
