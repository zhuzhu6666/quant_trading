...
> Status: active
> Last verified: 2026-09-14 (反事实复盘流解堵批：准入白名单纳入 `chain_broken`（`79ba699f`），learning worker 重启后首个 `supervisor_learning` 周期即为本日 15:30 监督员平仓产出复盘并验收；同日 post-fill 记录接线断裂已修复并在两笔真实成交上验收（`ORDER+AMEND OK` / attribution recorded / 学习样本 `integrity=full`），4 小时 latch 已按 cause 释放；因子发现闭环首次流动（21 retire + 11 promote、首个 `PROMOTION_PREPARED`），退出条件 (c) 未达；学习建议应用闭环 (b) 确认（`learning_same_direction_cooldown` 真实触发 2 次）；新增 `recovery_position_state.attribution_integrity` 无写入者条目（用户裁定下一批修）。此前各批追溯走 Git 历史)
...
- 状态：`monitoring`（2026-09-14 修复并受控重启验收：白名单纳入 `chain_broken`，提交 `79ba699f`；learning worker 重启后 10:09 UTC 的 `supervisor_learning` 周期即为 2026-09-14 15:30 的监督员平仓（position 288549050）产出首条新复盘 `live_counterfactual_scf_0375894ff3c82cbe_8d0eda3e7e15f0beb8fa`——`protection_too_tight` 0.74、`fully_matured`、`maturity.governance_eligible=true`、`selection_eligible=true`、模板绑定 `binding_verified`、`advisory_only=true`；事件数 198→199，最新由 09-11 00:35 推进到 09-14 15:42。退出：后续真实平仓持续产出干净成熟复盘并驱动 `position_supervisor_selection.v1` 出现首个可治理候选后转 resolved；仅 `broker_close`/无执行动作的平仓仍按设计判 `not_executed`）
...
- 修复（用户裁定选项①）：`backend/services/supervisor_counterfactual.py` 白名单纳入 `chain_broken`（注释说明判据：评的是执行动作而非触发证据，`_trace_is_real_execution` 与 `entry_price/close_price>0` 两道门继续兜底）；`tests/test_supervisor_counterfactual.py` 新增两例——`chain_broken` 产出复盘、未列白名单原因仍被过滤（注入错误过滤后前一例失败 0 != 1）。监督相关 8 个测试文件 79 passed。
- 验证：`run_artifacts/open_market_verify/probe_cf.py`（只读复算 + 单 review 诊断）、`run_artifacts/open_market_verify/probe_cf_newest.py`（读最新事件 payload）；`select count(*), max(observed_at) from canonical_v2.event where event_type='counterfactual_review'`；learning worker 日志 `[evolution_coordinator] finished supervisor_learning in 10.3s`。
