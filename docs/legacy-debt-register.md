...
- 当前口径: 统一 market session 返回 open_pending_quote 或 broker_connected_market_data_stale、API/连接健康且无 broker error 时，按 RuntimeConfig 的75分钟上限进入 maintenance_wait；快照复用最新 spot quote，正常 open、到期或断连状态仍严格判 stale；定时 data sync 与 live decision-bar 即时修复可遵守该窗口，但 stale/missing quote 的 spot 重订阅不得被 maintenance_wait 抑制，并由 serial live tick 在 bridge 晚就绪后限频恢复。
- 影响面: system health、data sync、live decision bar repair、spot subscription、告警和 readiness。
- 收口方式: 使用市场证据和限时宽限，不写死每日钟点；维护期抑制无效 K 线回补，但 stale/missing quote 仍以 60 秒限频补订阅，首个新 quote/bar 后自然恢复。
- 验证方式: `tests/test_market_maintenance_wait.py`、`tests/test_live_data_sync_job.py`、`tests/test_live_service_lifecycle.py::test_ensure_live_decision_bars_suppresses_repair_during_maintenance`、`tests/test_live_service_lifecycle.py::test_spot_subscription_refresh_continues_during_maintenance`、`tests/test_live_loop_tick_runtime.py`。
...
- 收口方式: 本轮已移出上述新增实现并保留薄兼容入口；session restore 的 deals-first 重建及 `available/degraded_cache/unavailable` 选择由 `session_restore.resolve_session_restore()` 纯函数拥有；旧 protection 仲裁已迁入 `live_position_protection_cycle`；recovery CRUD/replay/retirement 已迁入 `live_recovery_position_store`/`live_recovery_close`；close 后 cycle/processing 已迁入对应独立模块；open broker submission/draining/post-fill fail-closed、broker-confirmed SL/TP attach/projection-ack、filled/amended context/recovery/audit 分别迁入 `live_open_submission`、`live_open_protection`、`live_open_processing`；emergency fallback 已迁入 `live_execution_recovery`。`live_service` 对这些边界只注入 runtime callbacks。后续继续迁出其余 lifecycle/execution wiring；稳定发布后再删除 legacy compatibility authority 与旧 globals。
- 验证方式: façade 架构测试强制上述 wrapper 无内嵌控制流或决策文本；recovery/closed-position 系列测试覆盖 close 路径；`tests/test_live_open_submission.py` 覆盖 final admission、draining、RPC 异常、confirmed-open post-fill/reconcile 双失败；`tests/test_live_open_protection.py` 覆盖 projection verified/unverified、broker reject 与 amend exception；`tests/test_live_open_processing.py` 覆盖 ledger failure 后 recovery、success 顺序、辅助失败隔离及 latch-before-recovery；其余由 open admission、entry protection、generation/shutdown 回归。
