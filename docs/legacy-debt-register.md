...
- 收口方式: 本轮已移出上述新增实现并保留薄兼容入口；session restore 的 deals-first 重建及 `available/degraded_cache/unavailable` 选择由 `session_restore.resolve_session_restore()` 纯函数拥有；旧 protection 的 timeout/entry repair/supervisor/trailing 仲裁已迁入 `live_position_protection_cycle`；`recovery_position_state` CRUD 已迁入 `live_recovery_position_store`；recovered-close replay/retirement 已迁入 `live_recovery_close`；close 后 attribution/recovery/session fail-closed 编排已迁入 `live_closed_position_cycle`；emergency execution-intent fallback 已迁入 `live_execution_recovery`。`live_service` 对这些边界只注入 callback/connection/static flag runtime。后续继续迁出其余 lifecycle/execution wiring；稳定发布后再删除 legacy compatibility authority 与旧 globals。
- 验证方式: `tests/test_live_service_facade_boundaries.py` 强制 façade 无 recovery/post-close 决策文本，且 wrapper 无循环/异常实现；`tests/test_live_recovery_position_store.py`、`tests/test_live_recovery_close.py`、`tests/test_live_closed_position_cycle.py` 分别覆盖持久化、projection-before-release/fresh absence 与 post-close fail-closed/session rebuild；其余由 lifecycle/generation/emergency 回归。
