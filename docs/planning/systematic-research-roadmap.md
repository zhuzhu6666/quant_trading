...
结论：研究必须按"先测归因质量 → 再测监督有效性 → 再动因子"的顺序推进，否则前一层未量化的噪声会进入后一层的结论。
...
- 重任务执行：`runtime.jobs` 八类（backtest/discover/tuning/ab_test/external_refresh/sync/factor_health/parameter_template_validation），job worker 唯一执行，evolution_work_coordinator advisory lock 错峰。
- 研究证据信任边界：`research_evidence` 统一 fail-closed 校验；历史 parity 回测恒 `governance_eligible=false`，回测样本 `causal_level=simulated` 且只允许回测增强训练（权重 ≤ 真实、同 holdout 比较）。

## 3. 阶段路线（每阶段有证据门，允许跳过但不允许乱序）

### R0 基线测量（只读，先行）— **done，见 [research-r0-baseline-report.md](research-r0-baseline-report.md)**
- 结论：R1 与 R2 并列第一优先、R3 押后；监督呈"单一 close 路径 + 43.4% 过紧后验 + MFE 双峰"，
  归因层有 35.3% 污染率与未启用的责任域分解。细节以报告为准，本节不再复制。
...
### R1 归因质量研究（复盘层先行）
- 假设：当前最大的学习损失在归因层（污染/证据不足/责任域混淆），不在因子层。
- 方法：对 R0 发现的归因薄弱点逐个核实（如 entry_ts vs signal_bar_ts 混用、system_issue 误判、
  recovered/chain_broken 的 attribution_integrity 分布）；验证 `good_win/lucky_win/bad_loss` 标签与
  MFE/MAE/giveback 路径的一致性。
- 产出：归因改进证据 → 若发现系统性误标签，走 suppression/eligibility projection（不删事实）。
- 门槛：改进后 eligibility 通过率与责任域判定一致性可复算。

### R2 监督有效性研究
- 假设：监督模板的 tighten/reduce 触发点存在系统性过早/过晚，可用 counterfactual 后验证明。
- 方法：按绑定模板分层比较 path 指标（MFE capture/giveback/time_in_profit）与 counterfactual 判定；
  regime 条件化（复用 `project_current_market_regime()`）。
- 产出：监督模板改进候选（阈值/证据窗），走 supervisor 学习样本 → `position_supervisor_selection.v1`
  通道（自动进有界 Demo 的门槛是 ≥10 笔合格 matured + `tighten` 真实执行覆盖，rollout status §3.2；
  `reduce` 已永久关闭）。
- 门槛：每候选单 regime stratum、完整成熟反事实、无污染（position-supervisor-contract §7）。

### R3 因子研究
- 前提：R1 确认归因信号质量达标后，因子贡献结论才可信。
- 方法（现有通道内）：
  1. `backtest` job 验证 10 个外部低频因子的独立贡献与成本后净值；
  2. `discover`/GP 搜索继续（watermark gate 已防重复），优先排空 dsl_auto 积压（登记册：1712，源头节流生效中）；
  3. `tuning`/`ab_test` 用于参数与组合假设；
...
- 监督治理自动进 Demo 的门槛是数据积累，不是工程：开盘后确认 supervisor_evaluation/trade_review 事件流
  恢复即可，不人为制造 tighten。
...
- 不新建研究框架/表/线程/调度器/第二套回测；一切重任务走 `runtime.jobs` 现有八类。
