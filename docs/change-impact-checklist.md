...
| Execution outcome fault matrix | `execution_outcome_enable` 前是否持有 `execution_outcome_fault_matrix.v1` passed attestation；timeout/delayed/unknown/amend-no-ack/restart/intent-boundary/PG-independent-reduction 七类场景和 11 个 nodeid 是否完整；record hash 与当前 execution source/test binding 是否匹配；失败或代码变化后是否要求重跑；runner 是否不触碰 broker、配置或服务 |
