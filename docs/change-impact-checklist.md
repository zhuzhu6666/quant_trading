...
| 静态 rollout flags | safety/generation/execution/governance/job queue 是否与当前分期发布值一致（当前 Safety 为 `shadow`、governance 为 `dual_record`，其余仍为 false）；是否严格按 `safety_enforce -> generation_enable -> execution_outcome_enable -> governance_enforce -> pg_job_queue_enable` 每次只推进一个开关、只由发布配置+重启切换且不进入 RuntimeConfig overlay/自治 mutation；每一阶段的 `scripts/phased_repair_release_gate.py --target <target>` 是否精确校验前态并在观察门未达成、跳级或乱序时非零退出 |
| backend loaded flags | `generation_enable` 及后续 target 是否从新鲜持久化 readiness 读取 backend 进程实际加载的五项 static flags；values/fingerprint/PID/process-start 是否完整且与 CLI predecessor flags 精确一致；只改配置未重启时是否返回 `backend_process_static_flags_unconfirmed` |
...
| 持久化任务队列 | heavy job 是否只由独立 worker claim；claim token/heartbeat/lease/retry/cancel 是否可恢复；全局与 kind 并发限额是否跨进程生效；flag 默认 off 是否保持当前 demo；`pg_job_queue_enable` 前是否只读证明 canonical YAML/最低 schema 有效、JobManager 与 handler registry 一致、无 unsupported v1 runnable kind、无 pre-existing active lease |
