...
| 静态 rollout flags | safety/generation/execution/governance/job queue 五个开关是否仍为 off/false；是否只由发布配置+重启切换，不进入 RuntimeConfig overlay/自治 mutation；观察门未达成时是否拒绝 enforce |
