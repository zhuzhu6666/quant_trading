...
# glue deleted: canonical is backend.jobs.handlers
from backend.jobs import handlers as external_refresh
...
def test_external_refresh_worker_handler_is_bounded_and_reports_progress(
    ...
    monkeypatch.setattr(external_refresh, "REFRESH_SCRIPT", script)
    monkeypatch.setattr(
        external_refresh.subprocess,
        "run",
    ...
    result = external_refresh.run_external_refresh_job(
