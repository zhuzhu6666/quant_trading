...
def test_runtime_control_risk_is_derived_from_before_after() -> None:
    ...
    assert unfreeze.risk_class == "risk_tightening"


def test_system_actor_cannot_modify_operator_pause_kill_switch(
    ...
    result = RuntimeConfigMutationService(tmp_path / "state.db").apply_patch(
        {"governance_expansion_paused": True},
        source="pytest_operator_pause",
        actor="system:pytest",
        action="operator_pause_expansionary_autonomy",
        risk_reduction=True,
        audit=False,
    )
    ...
    try:
        ...
        assert "governance_mutation_intent" not in tables
    finally:
        conn.close()
