...
def _reset_state(monkeypatch):
    ...
    monkeypatch.setattr(live_close_settlement, "bounded_demo_mode_active", lambda: False)
    _reset_session_state()
    live_service.live_state_update(session_start_balance=1000.0)
    yield
    _reset_session_state()
    live_service.live_state_update(session_start_balance=1000.0)
...
def test_circuit_breaker_triggers_at_5_percent_drawdown():
    live_service.live_state_update(session_pnl=-50.0, session_start_balance=1000.0)

    result = live_close_settlement.evaluate_daily_drawdown(
...
def test_circuit_breaker_uses_risk_limit_snapshot_threshold():
    live_service.live_state_update(session_pnl=-40.0, session_start_balance=1000.0)
...
def test_demo_drawdown_is_observed_without_tripping_circuit(monkeypatch):
    monkeypatch.setattr(live_close_settlement, "bounded_demo_mode_active", lambda: True)
    live_service.live_state_update(
        session_pnl=-500.0,
...
def test_positive_pnl_does_not_trigger_breaker():
    live_service.live_state_update(session_pnl=100.0, session_start_balance=1000.0)
...
def test_pending_close_ids_track_deal_wait_without_touching_pnl():
    ...
    live_service.live_state_update(session_pending_close_add=99001)
    live_service.live_state_update(session_pending_close_add=[99002, 99001])
    ...
    live_service.live_state_update(session_pending_close_remove=[99001])
    ids = live_service.live_state_get("session_pending_close_ids")
    assert 99001 not in ids
    assert 99002 in ids
    live_service.live_state_update(session_pending_close_remove=[99002])
