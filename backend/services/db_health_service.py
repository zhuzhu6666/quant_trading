"""Database health snapshot service: size/row-count/freshness cache.

计算与缓存机制从 ``backend.api.db_health`` 平移至此：进程生命周期接线
（backend.services.backend_runtime_lifecycle）直接调用本模块的公共入口，
服务层不再反向依赖 API 层；API 模块只保留 HTTP 路由。
"""
import sqlite3
import threading
import time
...
def start_background_refresh(*, initial_delay_sec: float = 0.0) -> dict:
...
def stop_background_refresh(*, timeout_sec: float = 30.0) -> dict:
...
def snapshot_or_compute() -> dict:
    """返回缓存的健康快照；尚未预热时同步计算一次（仅冷启动发生）。"""
