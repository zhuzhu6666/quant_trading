...
- Unified Proposal Registry first implementation is complete: `ProposalRegistryService` writes `proposal_registry`, normalizes `policy_suggestion`、`brain_governance_candidate`、`brain_action_plan`、`learning_application_log`、`evolution_decision`、shadow/advisory audit and LLM advisory audit, exposes `/api/ops/autonomy/proposals*`, detects same control-surface conflicts, and review is audit-only. Status now uses a lightweight summary path and separates stale evidence into hard/replay/review queues so historical review work does not falsely block guarded demo apply. Web `/v16` shows the proposal bus without recomputing risk or authorizing actions.
- Autonomous self-evolution cycle visibility and first nursery runner are complete: `AutonomousEvolutionCycleService` exposes read-only `/api/ops/autonomy/evolution-cycle` and readiness `v16.autonomous_evolution_cycle`, while `AutonomousEvolutionNurseryRunner` exposes `/api/ops/autonomy/evolution-cycle/run` and the learning worker job `autonomous_evolution_nursery`. The runner reuses replay/release/effect/proposal/candidate-review/autonomous-learning services, defaults to repair/ready-only, exposes guarded apply windows, and can now consume at most one recommended demo apply step per nursery cycle. It still does not submit orders or bypass RiskPolicy/DecisionPolicy/runtime overlay. `apply_demo_autonomy` now requires explicit blocking confirmation; `AutonomousDemoApplyStepper` exposes `/api/ops/autonomy/demo-apply-plan` and confirmed one-step `/demo-apply-step`, with factor pruning split into explicit `materialize` / `promote` / `bridge` steps so routine nursery advancement does not run the heavy candidate rescan; full `run_autonomous_learning_cycle` is explicit via `full_learning_cycle=true`; nursery replay freshness is lightweight and DuckDB read-lock tolerant via snapshot fallback.
...
```text
AutonomousEvolutionCycle + nursery runner first loop complete; stabilize replay/release/effect freshness and effect-driven growth before increasing live autonomy permissions
```
...
- `AutonomousEvolutionCycle` is now the read-only self-evolution cycle map, `AutonomousEvolutionNurseryRunner` is the low-frequency coordinator that closes freshness blockers and consumes one recommended governance step per cycle, and `AutonomousDemoApplyStepper` is the explicit single-step bridge back to existing apply functions. On the live state checked 2026-07-08/09, the cycle reached `ready_for_guarded_demo_apply`; remaining proposal stale items were replay/review work queues, not hard blockers. Factor pruning stepper work is now stage-split, so `factor_pruning_materialize` can be scheduled separately from quick `factor_pruning_promote` / `factor_pruning_bridge` progression.
...
| G15 | Stable demo nursery self-evolution cycle needs production burn-in, replay-vs-bars DuckDB lock isolation, effect-driven reinforce/rollback, Web surfacing, and live-candidate graduation rules | active | `docs/autonomous-trading-final-blueprint.md` |
