...
class BarFilter:
    ...
    用法:
        f = BarFilter(db_path="data/ctrader_data.duckdb")
    ...
    def _now_epoch(self) -> float:
        """返回当前 epoch, fallback 到本地时间。"""
        import time as _time
        return _time.time()

    def filter(self, bars: list[dict], symbol: str, timeframe: str,
               tf_minutes: int = 15, skip_dedup: bool = False) -> FilterResult:
        """
        ...
        # 1. 去重: 查 db 已有 bar 的最大 time (full_sync 回填时跳过此步)
        if not skip_dedup:
            ...
            if db_max_time is not None:
                ...
                bars = new_bars
                if dup > 0:
                    logger.info(f"[BarFilter] 去重: {dup} 个重复 bar (db 最新={datetime.fromtimestamp(db_max_time, tz=timezone.utc)})")
        ...
        if bar_end_time > now:
            logger.info(f"[BarFilter] 跳过当前正在形成的 bar "
                        f"(time={datetime.fromtimestamp(last_bar['time'], tz=timezone.utc)})")
            bars = bars[:-1]
            ...
            for i in range(1, len(bars)):
                ...
                if step < expected_step * 0.5 and gaps < 3:
                    logger.warning(f"[BarFilter] 时间间隔异常: "
                                   f"t={datetime.fromtimestamp(bars[i-1]['time'], tz=timezone.utc)} → "
                                   f"{datetime.fromtimestamp(bars[i]['time'], tz=timezone.utc)} "
    ...
    def _get_max_time(self, symbol: str, timeframe: str) -> Optional[float]:
        ...
        改用 DataStore (DuckDB) 而非直接 sqlite3, 确保与主存储一致。"""
        try:
            from data.store import DataStore
            store = DataStore(self.db_path)
            df = store.load_bars(symbol, timeframe, limit=1)
            if df is not None and len(df) > 0:
                ts = df.index[-1]
                return float(ts.timestamp()) if hasattr(ts, 'timestamp') else float(ts)
        except Exception as e:
            logger.warning(f"[BarFilter] DataStore 查 max_time 失败: {e}")
        return None
