...
def main():
    ...
    for i, bar in enumerate(bars):
        # 转 dict + time 字段 (P1-E bug 修复: 之前 Series 传下去, 事件过滤 fallback 到 epoch=0
        # → NFP/FOMC skip 永远不触发, PnL 被低估)
        if isinstance(bar, pd.Series):
            bar_dict = bar.to_dict()
            bar_dict["time"] = bar.name if hasattr(bar, "name") else 0
            # bar.name 是 pd.Timestamp, 转 int epoch (策略 utcfromtimestamp 期待 int)
