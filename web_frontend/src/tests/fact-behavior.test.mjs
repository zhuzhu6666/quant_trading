...
assert.equal(factBoundTone(retainedHealthyFact, "ok"), "warn", "stale retained success must not stay green");
...
assert.equal(factBoundTone(retainedPnlFact, "ok"), "warn", "retained positive PnL must not remain green");
...
for (const contract of ["ops.backend-readiness.v2", "risk.summary.v2"]) {
  ...
  assert.equal(factBoundTone(retainedV15Fact, "ok"), "warn", `${contract} retained success must not stay green in V15`);
