...
import { getReadinessView, getRiskSnapshot } from "@/api/workbench";
...
export function SafetyRail({ onRefresh }: { onRefresh: () => void }) {
  ...
  const risk = useQuery({ queryKey: ["workbench", "risk"], queryFn: getRiskSnapshot, staleTime: 15_000, refetchInterval: 20_000, refetchIntervalInBackground: true, retry: false });
