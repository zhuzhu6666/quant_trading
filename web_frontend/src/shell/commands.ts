...
export function buildCommands({ workspace, live, readiness, navigate, refreshFacts, onStop, onEmergency }: {
  ...
  return [
    ...
    { id: "nav.workflow", label: `打开${workspaceLabels.workflow.label}`, description: "查看当前项目的实时运行路径和组件职责", shortcut: "Ctrl/Cmd+6", workspace: "workflow", riskClass: "read-only", enabled: workspace !== "workflow", execute: go("workflow") },
